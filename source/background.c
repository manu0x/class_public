...
int background_functions(
  ...
  if (pba->has_tach_insq == _TRUE_) {
    phi = pvecback_B[pba->index_bi_phi_tach_insq];
    phi_prime = pvecback_B[pba->index_bi_phi_prime_tach_insq];
    pvecback[pba->index_bg_phi_tach_insq] = phi; // value of the scalar field phi
    pvecback[pba->index_bg_phi_prime_tach_insq] = phi_prime; // value of the scalar field phi derivative wrt conformal time
    pvecback[pba->index_bg_V_tach_insq] = V_tach_insq(pba,phi); //V_tach_insq(pba,phi); //write here potential as function of phi
    pvecback[pba->index_bg_dV_tach_insq] = dV_tach_insq(pba,phi); // dV_tach_insq(pba,phi); //potential' as function of phi
    pvecback[pba->index_bg_ddV_tach_insq] = ddV_tach_insq(pba,phi); // ddV_tach_insq(pba,phi); //potential'' as function of phi
    pvecback[pba->index_bg_rho_tach_insq] = V_tach_insq(pba,phi)/( 3.0*sqrt(1.0-phi_prime*phi_prime/(a*a)) ); // energy of the scalar field. The field units are set automatically by setting the initial conditions
    pvecback[pba->index_bg_p_tach_insq] = -V_tach_insq(pba,phi)*sqrt(1.0-phi_prime*phi_prime/(a*a))/3.0 ; // pressure of the scalar field
    rho_tot += pvecback[pba->index_bg_rho_tach_insq];
    p_tot += pvecback[pba->index_bg_p_tach_insq];
    dp_dloga += 0.0; /** <-- This depends on a_prime_over_a, so we cannot add it now! */
    //divide relativistic & nonrelativistic (not very meaningful for oscillatory models)
    rho_r += 3.*pvecback[pba->index_bg_p_tach_insq]; //field pressure contributes radiation
    rho_m += pvecback[pba->index_bg_rho_tach_insq] - 3.* pvecback[pba->index_bg_p_tach_insq]; //the rest contributes matter
...
if (pba->has_tach_insq == _TRUE_){
    /** The contribution of tach_insq was not added to dp_dloga, add p_tach_insq_prime here: */
    pvecback[pba->index_bg_p_prime_tach_insq] = sqrt(1.0 - pvecback[pba->index_bg_phi_prime_tach_insq]*pvecback[pba->index_bg_phi_prime_tach_insq]/(a*a))
...
int background_free_input(
 ...
 if (pba->Omega0_tach_insq != 0.){
    if (pba->tach_insq_parameters != NULL)
...
int background_indices(
  ...
  class_define_index(pba->index_bg_phi_tach_insq,pba->has_tach_insq,index_bg,1);
  class_define_index(pba->index_bg_phi_prime_tach_insq,pba->has_tach_insq,index_bg,1);
  class_define_index(pba->index_bg_V_tach_insq,pba->has_tach_insq,index_bg,1);
  class_define_index(pba->index_bg_dV_tach_insq,pba->has_tach_insq,index_bg,1);
  class_define_index(pba->index_bg_ddV_tach_insq,pba->has_tach_insq,index_bg,1);
...
int background_solve(
  ...
  if (pba->background_verbose > 2) {
    ...
    if (pba->has_tach_insq == _TRUE_){
      printf("    Scalar field details:\n");
      printf("     -> Omega_tach_insq = %g, wished %g\n",
             pvecback[pba->index_bg_rho_tach_insq]/pvecback[pba->index_bg_rho_crit], pba->Omega0_tach_insq);
      if(pba->has_lambda == _TRUE_)
        printf("     -> Omega_Lambda = %g, wished %g\n",
               pvecback[pba->index_bg_rho_lambda]/pvecback[pba->index_bg_rho_crit], pba->Omega0_lambda);
      printf("     -> parameters: [n,phi_i,phi_prime_i] = \n");
      printf("                    [");
      for (i=0; i<pba->tach_insq_parameters_size-1; i++){
        printf("%.3f, ",pba->tach_insq_parameters[i]);
      }
...
int background_initial_conditions(
  ...
  */
  ...
  if(pba->has_tach_insq == _TRUE_){
    tach_insq_n = pba->tach_insq_parameters[0];
  
 
      pvecback_integration[pba->index_bi_phi_tach_insq] = pba->phi_ini_tach_insq;
      pvecback_integration[pba->index_bi_phi_prime_tach_insq] = pba->phi_prime_ini_tach_insq;
    
    class_test(!isfinite(pvecback_integration[pba->index_bi_phi_tach_insq]) ||
               !isfinite(pvecback_integration[pba->index_bi_phi_tach_insq]),
               pba->error_message,
               "initial phi = %e phi_prime = %e -> check initial conditions",
...
int background_output_titles(struct background * pba,
  ...
  class_store_columntitle(titles,"(.)rho_tach_insq",pba->has_tach_insq);
  class_store_columntitle(titles,"(.)p_tach_insq",pba->has_tach_insq);
  class_store_columntitle(titles,"(.)p_prime_tach_insq",pba->has_tach_insq);
  class_store_columntitle(titles,"phi_tach_insq",pba->has_tach_insq);
  class_store_columntitle(titles,"phi'_tach_insq",pba->has_tach_insq);
  class_store_columntitle(titles,"V_tach_insq",pba->has_tach_insq);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_tach_insq],pba->has_tach_insq,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_tach_insq],pba->has_tach_insq,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_prime_tach_insq],pba->has_tach_insq,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_phi_tach_insq],pba->has_tach_insq,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_phi_prime_tach_insq],pba->has_tach_insq,storeidx);
...
int background_derivs(
  ...
  if (pba->has_tach_insq == _TRUE_){
    /** - Scalar field equation: \f$ \phi'' + 2 a H \phi' + a^2 dV = 0 \f$  (note H is wrt cosmic time) */
    dy[pba->index_bi_phi_tach_insq] = y[pba->index_bi_phi_prime_tach_insq];
    dy[pba->index_bi_phi_prime_tach_insq] = -(y[pba->index_bi_a]*y[pba->index_bi_a]*dV_tach_insq(pba,y[pba->index_bi_phi_tach_insq])/
             V_tach_insq(pba,y[pba->index_bi_phi_tach_insq]))*(1.0 - y[pba->index_bi_phi_prime_tach_insq]*y[pba->index_bi_phi_prime_tach_insq]/
               (y[pba->index_bi_a]*y[pba->index_bi_a]) ) - 2.0*pvecback[pba->index_bg_H]*y[pba->index_bi_a]*y[pba->index_bi_phi_prime_tach_insq]
...
/**	Tachyon potentials and derivatives	**/
double V_tach_insq(struct background *pba,double phi) 
{
  double n  = pba->tach_insq_parameters[0];
  double cn = 2.0*n*sqrt(1.0 -2.0/(3.0*n));


  return  cn/(phi*phi);
}

double dV_tach_insq(struct background *pba,double phi) 
{
  double n  = pba->tach_insq_parameters[0];
  double cn = 2.0*n*sqrt(1.0 -2.0/(3.0*n));


  return  -2.0*cn/(phi*phi*phi);

 
}

double ddV_tach_insq(struct background *pba,double phi) 
{
  double n  = pba->tach_insq_parameters[0];
  double cn = 2.0*n*sqrt(1.0 -2.0/(3.0*n));
